...
class VecDB:
    ...
    def _build_index(self, n_coarse=256, m=8, bits=8, batch_size=5000):
        ...
        print("ids_before:", lists[0][:] if lists[0] else [])
        print("codes_before:", codes[lists[0][:]] if lists[0] else [])
        ...
        with open(ivf_indices_file, "wb") as f_ids, open(ivf_codes_file, "wb") as f_codes:
            for c in range(n_coarse):
                ...
                if c==0:
                    print("ids_after:", ids_sorted[:10])
                    print("codes_after:", subset_codes[:10])
