...
class IVF_PQ_Index:
    ...
    def retreive(self,query_vector,cosine_similarity,get_row,n_clusters=3,n_neighbors=10):
        ...
        vectors = np.empty((0, ))
        self.start_offsets=np.memmap(os.path.join(self.folder_path,'start_offsets.dat'), dtype=np.uint32, mode='r', shape=(self.n_clusters,))
        ...
        for cluster in nearest_clusters:
            ...
            bin_path = os.path.join(self.folder_path, "codebook_packed.bin")

            item_size = self.m // 2          # bytes per vector
            offset_bytes = start * item_size
            block_len = (end - start) * item_size

            with open(bin_path, "rb") as f:
                f.seek(offset_bytes)
                buf = f.read(block_len)

            codewords = np.frombuffer(buf, dtype=np.uint8).reshape(end - start, item_size)        
            
            codewords=self.unpack_packed_codes(codewords)
    ...
    def save_index(self):
        ...
        mmap.flush()      
        mmap=np.memmap(os.path.join(folder_path,'start_offsets.dat'), dtype=np.uint32, mode='w+', shape=(self.n_clusters,))
        ...
        for label,codewords in self.inverted_index.items():
            ...
            all_indices.extend(codewords[1])
        mmap=np.memmap(os.path.join(folder_path,'all_indices.dat'), dtype=np.uint32, mode='w+', shape=(len(all_indices),))
        mmap[:]=all_indices
        mmap.flush()
        all_codes_words = np.asarray(all_codes_words, dtype=np.uint8)
        packed_codebook = np.empty((self.db_size, self.m // 2), dtype=np.uint8)

        for i in range(self.m // 2):
            packed_codebook[:, i] = (all_codes_words[:, 2*i] << 4) | all_codes_words[:, 2*i + 1]

        packed_codebook = np.ascontiguousarray(packed_codebook)
        packed_codebook.tofile(os.path.join(self.folder_path, "codebook_packed.bin"))
