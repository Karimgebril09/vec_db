...
class IVF_PQ_Index:
    ...
    def retreive(self,query_vector,cosine_similarity,get_row,n_clusters=3,n_neighbors=10):
        ...
        for cluster in nearest_clusters:
            ...
            indices_start_offsets=np.memmap(os.path.join(self.folder_path,'start_offsets_indices.bin'), dtype=np.uint32, mode='r', shape=(self.n_clusters,))
            indices=decode_single_list(os.path.join(self.folder_path,'all_indices.bin'), indices_start_offsets[cluster])            
